...
// Inicializálja az API-t a localStorage-ból származó kulccsal
export const initializeGeminiAPI = (apiKey: string) => {
  ai = new GoogleGenAI({ apiKey });
};

// Ellenőrzi, hogy az API inicializálva van-e
const ensureAPIInitialized = () => {
  if (!ai) {
    const apiKey = localStorage.getItem('GEMINI_API_KEY');
    if (!apiKey) {
      throw new Error('API kulcs nincs beállítva. Kérlek, add meg az API kulcsot.');
    }
